...
import { useF41v4_DashboardStore, type F41v4_WindowState, type F41v4_Adjustment, type F41v4_LeaveRequest } from '@/stores/F41v4_DashboardStore';
...
const getStatusBadge = (windowState: F41v4_WindowState, confirmed: boolean) => {
  if (windowState === 'PAID') {
...
const getAdjustmentStatusColor = (status: F41v4_Adjustment['status']) => {
  switch (status) {
    case 'Pending':
...
const getLeaveStatusColor = (status: F41v4_LeaveRequest['status']) => {
  switch (status) {
    case 'Pending':
      return 'bg-amber-100 text-amber-700 border-amber-300 dark:bg-amber-500/15 dark:text-amber-500 dark:border-amber-500/30';
    case 'Admin approved':
...
export const F41v4_UpcomingPayCard = () => {
  ...
  const [selectedAdjustment, setSelectedAdjustment] = useState<F41v4_Adjustment | null>(null);
      ...
      <Card className="border border-border/40 shadow-sm bg-card/50 backdrop-blur-sm">
        <CardHeader className="bg-gradient-to-r from-primary/[0.02] to-secondary/[0.02] border-b border-border/40">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <CardTitle className="text-lg">{isPaid ? 'Last payment' : 'Upcoming pay'}</CardTitle>
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-lg bg-gradient-to-br from-primary/[0.05] to-secondary/[0.03] border border-border/30">
              ...
              <p className="text-2xl font-semibold text-foreground">{formatCurrency(estimatedNet, currency)}</p>
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent><p className="text-xs">Some items are controlled by country rules</p></TooltipContent>
                      </Tooltip>
                    </div>
                    <p className="text-lg font-medium text-foreground">{formatCurrency(totalEmployerCosts, currency)}</p>
                  </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {item.locked && (
                        <Tooltip>
                          ...
                          <TooltipContent><p className="text-xs">Defined by country configuration</p></TooltipContent>
                        </Tooltip>
                      )}
                    </div>
                    <span className="text-accent-green-text font-medium">+{formatCurrency(item.amount, currency)}</span>
                  </div>
                ))}
              </div>
              <div className="space-y-2">
                ...
                {deductions.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between text-sm p-2 rounded bg-muted/20">
                    ...
                    <span className="text-destructive font-medium">{formatCurrency(item.amount, currency)}</span>
                  </div>
                ))}
              </div>
            </CollapsibleContent>
          ...
          <div className="space-y-2">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Your changes (this cycle)</p>
                ...
                <p className="text-sm text-muted-foreground">
                  {adjustments.length > 0 && (
                    <span>
                      ...
                      {adjustments.filter(a => a.status === 'Admin approved').length > 0 && ` · ${adjustments.filter(a => a.status === 'Admin approved').length} approved`}
                    </span>
                  )}
                  {adjustments.length > 0 && leaveRequests.length > 0 && ' · '}
                  {leaveRequests.length > 0 && (
                    <span>
                      ...
                      {leaveRequests.filter(l => l.status === 'Admin approved').length > 0 && ` · ${leaveRequests.filter(l => l.status === 'Admin approved').length} approved`}
                    </span>
                  )}
                </p>
                <div className="flex flex-wrap gap-2" role="list" aria-label="Your changes this cycle">
                  {adjustments.map((adj) => (
                    <div key={adj.id} className={cn('group inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border transition-all', getAdjustmentStatusColor(adj.status), isRemovable(adj.status) ? 'pr-1.5' : '')} role="listitem">
                      <button onClick={() => setSelectedAdjustment(adj)} className="inline-flex items-center gap-1.5 focus:outline-none">
                        <span>{adj.type}</span>
                        {adj.amount !== null && (<><span aria-hidden="true">·</span><span>{formatCurrency(adj.amount, currency)}</span></>)}
                        {adj.type === 'Overtime' && adj.hours && (<><span aria-hidden="true">·</span><span>{adj.hours}h</span></>)}
                      ...
                      {isRemovable(adj.status) && (
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <button onClick={(e) => handleWithdrawClick(e, 'adjustment', adj.id)} className="ml-0.5 p-0.5 rounded-full opacity-0 group-hover:opacity-100 hover:bg-black/10 dark:hover:bg-white/10 transition-opacity focus:opacity-100 focus:outline-none focus:ring-1 focus:ring-current" aria-label="Withdraw request"><X className="h-3 w-3" /></button>
                          </TooltipTrigger>
                          <TooltipContent side="top" className="text-xs">Withdraw request</TooltipContent>
                        </Tooltip>
                      )}
                    </div>
                  ))}
                  {leaveRequests.map((leave) => (
                    <div key={leave.id} className={cn('group inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border transition-all', getLeaveStatusColor(leave.status), isRemovable(leave.status) ? 'pr-1.5' : '')} role="listitem">
                      ...
                      {isRemovable(leave.status) && (
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <button onClick={(e) => handleWithdrawClick(e, 'leave', leave.id)} className="ml-0.5 p-0.5 rounded-full opacity-0 group-hover:opacity-100 hover:bg-black/10 dark:hover:bg-white/10 transition-opacity focus:opacity-100 focus:outline-none focus:ring-1 focus:ring-current" aria-label="Withdraw request"><X className="h-3 w-3" /></button>
                          </TooltipTrigger>
                          <TooltipContent side="top" className="text-xs">Withdraw request</TooltipContent>
                        </Tooltip>
                      )}
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            <Button onClick={() => setConfirmDialogOpen(true)} disabled={!isWindowOpen || confirmed} className="flex-1">{confirmed ? 'Pay Confirmed' : 'Confirm pay'}</Button>
            <Button variant="outline" onClick={() => setAdjustmentModalOpen(true)} disabled={isWindowClosed || isPaid} className="flex-1">Request change</Button>
          </div>

          <div className="text-center">
            <Button variant="link" onClick={() => setPayslipDrawerOpen(true)} className="text-sm text-muted-foreground hover:text-foreground">View previous payslips</Button>
          </div>
        </CardContent>
      </Card>

      <F41v4_AdjustmentModal open={adjustmentModalOpen} onOpenChange={setAdjustmentModalOpen} currency={currency} />
