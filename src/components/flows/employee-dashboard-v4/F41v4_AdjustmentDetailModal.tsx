...
import { type F41v4_Adjustment } from '@/stores/F41v4_DashboardStore';
import { FileText, Clock } from 'lucide-react';

interface F41v4_AdjustmentDetailModalProps {
  adjustment: F41v4_Adjustment | null;
  onClose: () => void;
  currency: string;
}
...
const getStatusBadge = (status: F41v4_Adjustment['status']) => {
  switch (status) {
    case 'Pending': return <Badge className="bg-amber-500/20 text-amber-400 border-amber-500/30">Pending</Badge>;
    case 'Admin approved': return <Badge className="bg-accent-green/20 text-accent-green-text border-accent-green/30">Admin approved</Badge>;
    case 'Admin rejected': return <Badge className="bg-destructive/20 text-destructive border-destructive/30">Admin rejected</Badge>;
    case 'Queued for next cycle': return <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30">Queued for next cycle</Badge>;
    default: return <Badge variant="secondary">{status}</Badge>;
  }
};

export const F41v4_AdjustmentDetailModal = ({ adjustment, onClose, currency }: F41v4_AdjustmentDetailModalProps) => {
            ...
            <div className="space-y-2">
              ...
              {isImage ? (
                <div className="rounded-lg overflow-hidden border border-border">
                  <img src={adjustment.receiptUrl} alt="Receipt" className="w-full max-h-48 object-cover" />
                </div>
              ) : (
                <a href={adjustment.receiptUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 rounded-lg bg-muted/50 border border-border hover:bg-muted transition-colors">
