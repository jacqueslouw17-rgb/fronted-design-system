...
interface CA4_TrackingViewProps {
...
export const CA4_TrackingView: React.FC<CA4_TrackingViewProps> = ({
  ...
  const getStatusConfig = (status: WorkerPaymentStatus) => {
    switch (status) {
      ...
      case "processing":
        return { icon: Clock, color: "text-primary", bg: "bg-primary/10", label: "Processing" };
      case "queued":
      case "sent":
        return { icon: Clock, color: "text-muted-foreground", bg: "bg-muted", label: "In progress" };
      case "failed":
        return { icon: AlertTriangle, color: "text-accent-amber-text", bg: "bg-accent-amber-fill/10", label: isHistorical ? "Not paid" : "Attention" };
    }
  };
            ...
            {isHistorical ? (
              ...
              <>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-primary/10 text-primary text-xs font-medium">In review</span>
                </div>
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl font-semibold text-foreground tabular-nums">{paidCount}</span>
                  <span className="text-lg text-muted-foreground">of {workers.length}</span>
        ...
        {!isHistorical && (
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-4 text-xs">
              ...
              {notPaidCount > 0 && (
                <div className="flex items-center gap-1.5">
                  <div className="h-2 w-2 rounded-full bg-accent-amber-fill" />
            ...
            return (
              <div 
                ...
                className={cn(
                  ...
                  needsAttention && "border-accent-amber-outline/30 bg-accent-amber-fill/5"
                )}
              >
                {/* Avatar */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {needsAttention && worker.errorMessage && (
                    <p className="text-[10px] text-accent-amber-text truncate leading-tight">{worker.errorMessage}</p>
                  )}
                </div>
...
export default CA4_TrackingView;
