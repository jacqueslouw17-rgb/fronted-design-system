...
import { useF41v7n_DashboardStore, type F41v7n_LeaveRequest } from '@/stores/F41v7n_DashboardStore';
import { format } from 'date-fns';
import { cn } from '@/lib/utils';
import { F41v7n_WithdrawDialog } from './F41v7n_WithdrawDialog';
import { toast } from 'sonner';

interface F41v7n_TimeOffSectionProps {
...
interface ProcessedLeave {
  ...
  status: F41v7n_LeaveRequest['status'];
...
export const F41v7n_TimeOffSection = ({ onRequestTimeOff }: F41v7n_TimeOffSectionProps) => {
  const { leaveRequests, withdrawLeaveRequest, payrollStatus } = useF41v7n_DashboardStore();
  ...
  const getStatusBadge = (status: F41v7n_LeaveRequest['status']) => {
    switch (status) {
      case 'Pending':
        return (
