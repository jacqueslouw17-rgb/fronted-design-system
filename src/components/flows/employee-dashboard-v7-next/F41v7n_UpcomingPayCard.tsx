...
import { useF41v7n_DashboardStore, type F41v7n_PayrollStatus, type F41v7n_Adjustment, type F41v7n_LeaveRequest } from '@/stores/F41v7n_DashboardStore';
import { F41v7n_AdjustmentModal, type RequestType } from './F41v7n_AdjustmentModal';
import { F41v7n_ConfirmPayDialog } from './F41v7n_ConfirmPayDialog';
import { F41v7n_SubmitNoChangesDialog } from './F41v7n_SubmitNoChangesDialog';
import { F41v7n_AdjustmentDetailModal } from './F41v7n_AdjustmentDetailModal';
import { F41v7n_PayslipHistoryDrawer } from './F41v7n_PayslipHistoryDrawer';
import { F41v7n_WithdrawDialog } from './F41v7n_WithdrawDialog';
import { F41v7n_WithdrawSubmissionDialog } from './F41v7n_WithdrawSubmissionDialog';
import { F41v7n_PayBreakdownDrawer } from './F41v7n_PayBreakdownDrawer';
...
const getStatusConfig = (status: F41v7n_PayrollStatus): {
  label: string;
  className: string;
  explanation: string;
  helperText?: string;
...
const getAdjustmentStatusColor = (status: F41v7n_Adjustment['status']) => {
  switch (status) {
    case 'Pending':
...
const getLeaveStatusColor = (status: F41v7n_LeaveRequest['status']) => {
  switch (status) {
    case 'Pending':
      return 'bg-amber-100 text-amber-700 border-amber-300 dark:bg-amber-500/15 dark:text-amber-500 dark:border-amber-500/30';
    case 'Admin approved':
...
export const F41v7n_UpcomingPayCard = () => {
  ...
  const [selectedAdjustment, setSelectedAdjustment] = useState<F41v7n_Adjustment | null>(null);
  ...
  return <>
        ...
        <CardContent className="px-4 sm:px-6 py-4 sm:py-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div 
              ...
              className="p-4 sm:p-5 rounded-xl border cursor-pointer transition-all duration-200 bg-gradient-to-br from-primary/[0.06] to-secondary/[0.04] border-border/40 hover:from-primary/[0.12] hover:to-secondary/[0.08] hover:border-border/60"
            >
            ...
            {/* Paid Date Tile */}
            <div onClick={() => setPayslipDrawerOpen(true)} className="p-4 sm:p-5 rounded-xl bg-muted/30 border border-border/40 cursor-pointer transition-all duration-200 hover:bg-muted/70 hover:border-border/60">
      ...
      {/* Modals & Drawers */}
      <F41v7n_AdjustmentModal open={adjustmentModalOpen} onOpenChange={handleAdjustmentModalClose} currency={currency} initialType={adjustmentModalInitialType} initialExpenseCategory={adjustmentModalInitialCategory} initialExpenseAmount={adjustmentModalInitialAmount} onBack={adjustmentModalFromBreakdown ? () => setBreakdownDrawerOpen(true) : undefined} />

      <F41v7n_ConfirmPayDialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen} periodLabel={periodLabel} />

      <F41v7n_SubmitNoChangesDialog open={noChangesDialogOpen} onOpenChange={setNoChangesDialogOpen} periodLabel={periodLabel} />

      <F41v7n_AdjustmentDetailModal adjustment={selectedAdjustment} onClose={() => setSelectedAdjustment(null)} currency={currency} />

      <F41v7n_PayslipHistoryDrawer open={payslipDrawerOpen} onOpenChange={setPayslipDrawerOpen} />

      <F41v7n_PayBreakdownDrawer open={breakdownDrawerOpen} onOpenChange={setBreakdownDrawerOpen} lineItems={lineItems} currency={currency} estimatedNet={estimatedNet} periodLabel={periodLabel} />

      <F41v7n_WithdrawDialog open={withdrawDialogOpen} onOpenChange={setWithdrawDialogOpen} onConfirm={handleConfirmWithdraw} requestType={withdrawTarget?.type || 'adjustment'} />

      <F41v7n_WithdrawSubmissionDialog open={withdrawSubmissionDialogOpen} onOpenChange={setWithdrawSubmissionDialogOpen} onConfirm={withdrawSubmission} />
    </>;
};
