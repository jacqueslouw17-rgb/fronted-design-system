...
import { useF41v7n_DashboardStore, type F41v7n_LeaveType } from '@/stores/F41v7n_DashboardStore';
...
interface F41v7n_TimeOffRequestDrawerProps {
  open: boolean;
...
const leaveTypes: { value: F41v7n_LeaveType; label: string; icon: string }[] = [
...
export const F41v7n_TimeOffRequestDrawer = ({ 
  open, 
  onOpenChange, 
  rejectedId, 
  rejectionReason,
  ...
  initialStartDate,
  initialEndDate
}: F41v7n_TimeOffRequestDrawerProps) => {
  const { addLeaveRequest, employeeCountry, leaveRequests, withdrawLeaveRequest } = useF41v7n_DashboardStore();
  
  const [countryRules, setCountryRules] = useState<CountryRules | null>(null);
  const [isLoadingRules, setIsLoadingRules] = useState(false);
  const [leaveType, setLeaveType] = useState<F41v7n_LeaveType>('Vacation');
