...
import { useF41v8_DashboardStore, type F41v8_LeaveType } from '@/stores/F41v8_DashboardStore';
...
interface F41v8_TimeOffRequestDrawerProps {
  open: boolean;
...
const leaveTypes: { value: F41v8_LeaveType; label: string; icon: string }[] = [
...
export const F41v8_TimeOffRequestDrawer = ({ 
  open, 
  onOpenChange, 
  rejectedId, 
  rejectionReason,
  ...
  initialStartDate,
  initialEndDate
}: F41v8_TimeOffRequestDrawerProps) => {
  const { addLeaveRequest, employeeCountry, leaveRequests, withdrawLeaveRequest } = useF41v8_DashboardStore();
  
  const [countryRules, setCountryRules] = useState<CountryRules | null>(null);
  const [isLoadingRules, setIsLoadingRules] = useState(false);
  const [leaveType, setLeaveType] = useState<F41v8_LeaveType>('Vacation');
