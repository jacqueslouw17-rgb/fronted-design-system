...
import { useF41v8_DashboardStore, type F41v8_LeaveRequest } from '@/stores/F41v8_DashboardStore';
import { format } from 'date-fns';
import { cn } from '@/lib/utils';
import { F41v8_WithdrawDialog } from './F41v8_WithdrawDialog';
import { toast } from 'sonner';

interface F41v8_TimeOffSectionProps {
...
interface ProcessedLeave {
  ...
  status: F41v8_LeaveRequest['status'];
...
export const F41v8_TimeOffSection = ({ onRequestTimeOff }: F41v8_TimeOffSectionProps) => {
  const { leaveRequests, withdrawLeaveRequest, payrollStatus } = useF41v8_DashboardStore();
  ...
  const getStatusBadge = (status: F41v8_LeaveRequest['status']) => {
    switch (status) {
      case 'Pending':
        return (
