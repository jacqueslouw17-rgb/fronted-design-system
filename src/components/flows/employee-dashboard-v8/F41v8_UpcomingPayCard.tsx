...
import { useF41v8_DashboardStore, type F41v8_PayrollStatus, type F41v8_Adjustment, type F41v8_LeaveRequest } from '@/stores/F41v8_DashboardStore';
import { F41v8_AdjustmentModal, type RequestType } from './F41v8_AdjustmentModal';
import { F41v8_ConfirmPayDialog } from './F41v8_ConfirmPayDialog';
import { F41v8_SubmitNoChangesDialog } from './F41v8_SubmitNoChangesDialog';
import { F41v8_AdjustmentDetailModal } from './F41v8_AdjustmentDetailModal';
import { F41v8_PayslipHistoryDrawer } from './F41v8_PayslipHistoryDrawer';
import { F41v8_WithdrawDialog } from './F41v8_WithdrawDialog';
import { F41v8_WithdrawSubmissionDialog } from './F41v8_WithdrawSubmissionDialog';
import { F41v8_PayBreakdownDrawer } from './F41v8_PayBreakdownDrawer';
...
const getStatusConfig = (status: F41v8_PayrollStatus): {
  label: string;
  className: string;
  explanation: string;
  helperText?: string;
...
const getAdjustmentStatusColor = (status: F41v8_Adjustment['status']) => {
  switch (status) {
    case 'Pending':
...
const getLeaveStatusColor = (status: F41v8_LeaveRequest['status']) => {
  switch (status) {
    case 'Pending':
      return 'bg-amber-100 text-amber-700 border-amber-300 dark:bg-amber-500/15 dark:text-amber-500 dark:border-amber-500/30';
    case 'Admin approved':
...
export const F41v8_UpcomingPayCard = () => {
  ...
  const [selectedAdjustment, setSelectedAdjustment] = useState<F41v8_Adjustment | null>(null);
  ...
  return <>
      ...
      {/* Modals & Drawers */}
      <F41v8_AdjustmentModal open={adjustmentModalOpen} onOpenChange={handleAdjustmentModalClose} currency={currency} initialType={adjustmentModalInitialType} initialExpenseCategory={adjustmentModalInitialCategory} initialExpenseAmount={adjustmentModalInitialAmount} onBack={adjustmentModalFromBreakdown ? () => setBreakdownDrawerOpen(true) : undefined} />

      <F41v8_ConfirmPayDialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen} periodLabel={periodLabel} />

      <F41v8_SubmitNoChangesDialog open={noChangesDialogOpen} onOpenChange={setNoChangesDialogOpen} periodLabel={periodLabel} />

      <F41v8_AdjustmentDetailModal adjustment={selectedAdjustment} onClose={() => setSelectedAdjustment(null)} currency={currency} />

      <F41v8_PayslipHistoryDrawer open={payslipDrawerOpen} onOpenChange={setPayslipDrawerOpen} />

      <F41v8_PayBreakdownDrawer open={breakdownDrawerOpen} onOpenChange={setBreakdownDrawerOpen} lineItems={lineItems} currency={currency} estimatedNet={estimatedNet} periodLabel={periodLabel} />

      <F41v8_WithdrawDialog open={withdrawDialogOpen} onOpenChange={setWithdrawDialogOpen} onConfirm={handleConfirmWithdraw} requestType={withdrawTarget?.type || 'adjustment'} />

      <F41v8_WithdrawSubmissionDialog open={withdrawSubmissionDialogOpen} onOpenChange={setWithdrawSubmissionDialogOpen} onConfirm={withdrawSubmission} />
    </>;
};
