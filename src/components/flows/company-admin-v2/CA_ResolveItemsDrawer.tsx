...
export const CA_ResolveItemsDrawer: React.FC<CA_ResolveItemsDrawerProps> = ({
  ...
  const [previewFile, setPreviewFile] = useState<{ name: string; type: "pdf" | "image" } | null>(null);
  ...
  const handleAttachmentClick = (fileName: string) => {
    ...
    setPreviewFile({ name: fileName, type: isImage ? "image" : "pdf" });
  };
  ...
  const getMockAdjustmentDetails = (adj: CA_Adjustment) => ({
    ...
    attachments: adj.hasReceipt ? [
      adj.type === "expense" ? "receipt.pdf" : null,
      adj.type === "overtime" ? "timesheet.png" : null,
                  ...
                  <TabsContent value="adjustments" className="px-6 py-4 space-y-3 m-0">
                    {filteredAdjustments.length === 0 ? (
                      ...
                      filteredAdjustments.map(adj => {
                        ...
                        return (
                          <Card key={adj.id} className="border-border/30 bg-card/50 overflow-hidden">
                                      ...
                                      <div className="flex flex-wrap gap-2">
                                        {details.attachments.map((file, idx) => (
                                          <button
                                            ...
                                            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-muted/50 hover:bg-muted text-xs text-foreground transition-colors"
                                          >
                  ...
                  <TabsContent value="leave" className="px-6 py-4 space-y-3 m-0">
                    {filteredLeave.length === 0 ? (
                      ...
                      filteredLeave.map(leave => {
                        ...
                        return (
                          <Card key={leave.id} className="border-border/30 bg-card/50 overflow-hidden">
                            <CardContent className="p-4">
                              <div className="flex items-start gap-3">
                                <Avatar className="h-9 w-9 flex-shrink-0">
                                  <AvatarFallback className="bg-primary/10 text-primary text-xs">
                                      ...
                                      {Array.from({ length: 7 }).map((_, idx) => {
                                        ...
                                        return (
                                          <div 
                                            key={idx}
                                            className={cn(
                                              "w-8 h-8 rounded-md flex flex-col items-center justify-center text-[10px]",
                                              isHighlighted 
                                    ...
                                    <p className="text-sm">
                                      {details.isPaidLeave ? (
                                        leave.impactAmount !== 0 ? (
                                          ...
                                          <span className="text-muted-foreground">Paid leave · no change to net pay</span>
                                        )
                                      ) : (
