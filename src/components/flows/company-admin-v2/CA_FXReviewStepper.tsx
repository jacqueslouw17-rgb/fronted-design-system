...
interface CA_FXReviewStepperProps {
  ...
  onStepClick?: (step: FXReviewStep) => void;
}
const stepConfig: {
  id: FXReviewStep;
  label: string;
...
export const CA_FXReviewStepper: React.FC<CA_FXReviewStepperProps> = ({
  ...
  return <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        const isClickable = state === "done" && onStepClick;
        ...
        return <React.Fragment key={step.id}>
              <button
                ...
                className={cn(
                  ...
                  // Completed: subtle primary styling with hover
                  state === "done" && "bg-primary/5 text-primary/80 font-medium border border-primary/20 hover:bg-primary/15 hover:border-primary/40 cursor-pointer",
                  // Future: very subtle, neutral
                  state === "todo" && "bg-muted/30 text-muted-foreground/70 font-normal cursor-not-allowed"
                )}
              >
