/**
 * Flow 5 v3 — Policy & Guardrails Setup Step
 * 
 * Isolated copy from Flow 1 v7 for Flow 5 Company Admin Onboarding v3 (Future).
 * DO NOT modify Flow 1 v7 or any other flow.
 */
...
const PolicySection = ({ 
  ...
  <div className={cn(
    "v7-glass-item overflow-hidden transition-all duration-200",
    ...
    <button type="button" onClick={onToggle} className="w-full flex items-center gap-3 px-4 py-3.5 text-left group">
      <div className={cn(
        ...
        isOpen ? "bg-muted/50 text-foreground/70" : "bg-muted/30 text-muted-foreground/50"
      )}>
        <Icon className="h-4 w-4" />
      </div>
      <div className="flex-1 min-w-0">
        <p className={cn("text-sm font-medium", isOpen ? "text-foreground" : "text-foreground/80")}>{title}</p>
        <p className="text-xs text-muted-foreground mt-0.5 truncate">{subtitle}</p>
      </div>
      <motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.2 }}>
        <ChevronDown className={cn("h-4 w-4 transition-colors", isOpen ? "text-muted-foreground/60" : "text-muted-foreground/30")} />
      </motion.div>
    </button>
    <AnimatePresence>
      {isOpen && (
...
const ThresholdSelector = ({
  label, value, customAmount, onChange, onCustomAmountChange,
}: {
  label: string;
  ...
  <div className="space-y-2">
    ...
    {value === "custom" && (
      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex items-center gap-2">
        ...
        <Input type="number" value={customAmount || ""} onChange={e => onCustomAmountChange(e.target.value)} placeholder="2500" className="w-24 h-8 text-xs" />
      </motion.div>
    )}
  </div>
);

const F5v3_PolicySetupStep = ({ formData, onComplete, isProcessing }: PolicySetupStepProps) => {
  const [policy, setPolicy] = useState<PolicyData>(() => ({ ...DEFAULT_POLICY, ...formData }));
  ...
  return (
    <div className="space-y-5 w-full sm:max-w-xl sm:mx-auto px-1 sm:px-0">
      <div className="space-y-2.5">
          ...
          <div className="space-y-4">
            ...
            <div className="v7-glass-item rounded-lg p-3 space-y-3">
              {policy.payrollCycle === "monthly" ? (
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    <Input type="number" min={1} max={31} value={policy.monthlyPayoutDay} onChange={e => updatePolicy("monthlyPayoutDay", e.target.value)} className="w-16 h-7 text-xs text-center" />
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                      <Input type="number" min={1} max={31} value={policy.fortnightlyPayoutDays[0]} onChange={e => updatePolicy("fortnightlyPayoutDays", [e.target.value, policy.fortnightlyPayoutDays[1]])} className="w-16 h-7 text-xs text-center" />
                      <span className="text-[10px] text-muted-foreground">of each month</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                      <Input type="number" min={1} max={31} value={policy.fortnightlyPayoutDays[1]} onChange={e => updatePolicy("fortnightlyPayoutDays", [policy.fortnightlyPayoutDays[0], e.target.value])} className="w-16 h-7 text-xs text-center" />
                      <span className="text-[10px] text-muted-foreground">of each month</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between border-t border-border/20 pt-3">
                ...
                <div className="flex items-center gap-1.5">
                  <Input type="number" min={1} max={15} value={policy.cutOffDaysBefore} onChange={e => updatePolicy("cutOffDaysBefore", e.target.value)} className="w-16 h-7 text-xs text-center" />
        ...
        >
          <div className="space-y-4">
            <ThresholdSelector label="Adjustment threshold" value={policy.adjustmentThreshold} customAmount={policy.adjustmentCustomAmount} onChange={v => updatePolicy("adjustmentThreshold", v)} onCustomAmountChange={v => updatePolicy("adjustmentCustomAmount", v)} />
            <ThresholdSelector label="Expense threshold" value={policy.expenseThreshold} customAmount={policy.expenseCustomAmount} onChange={v => updatePolicy("expenseThreshold", v)} onCustomAmountChange={v => updatePolicy("expenseCustomAmount", v)} />
            <div className="space-y-2">
            ...
            <AnimatePresence>
              {policy.escalationScope === "admin-plus-backup" && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="space-y-2">
                  ...
                  <Input type="email" value={policy.backupApproverEmail || ""} onChange={e => updatePolicy("backupApproverEmail", e.target.value)} placeholder="backup@company.com" className="text-sm" />
                </motion.div>
              )}
            </AnimatePresence>
            <div className="space-y-2">
...
export default F5v3_PolicySetupStep;
