...
const submissionTypeConfig: Record<SubmissionType, {
...
const leaveTypeConfig: Record<LeaveTypeLocal, {
  icon: React.ElementType;
  label: string;
  color: string;
  affectsPay: boolean;
...
const AdjustmentRow = ({
  ...
  const formatAmount = (amt: number, curr: string) => {
    ...
    return `${symbols[curr] || curr}${Math.abs(amt).toLocaleString(undefined, {
        ...
        ease: "easeOut"
            ...
            <div className="pb-3 space-y-2.5">
              ...
              handleApproveClick();
                ...
                </div> : <div className="space-y-2 p-3 rounded-md border border-border/50 bg-background/80" onClick={e => e.stopPropagation()}>
                ...
                setRejectReasonInput("");
                    ...
                    <Button size="sm" onClick={handleRejectClick} disabled={!rejectReasonInput.trim()} className="flex-1 h-8 text-xs bg-red-600 hover:bg-red-700 text-white shadow-none hover:shadow-none hover:translate-y-0">
                      Reject
                    </Button>
                  </div>
                </div>}
...
const BreakdownRow = ({
  ...
  return <div className={cn("flex items-center justify-between py-2", isTotal && "pt-3 mt-1 border-t border-dashed border-border/50", className)}>
      <span className={cn("truncate", isTotal ? "text-sm font-medium text-foreground" : "text-sm text-muted-foreground")}>
          {label}
        </span>
  ...
  if (isApproved) {
    return <div className={cn("flex items-center justify-between py-2.5 -mx-3 px-3 rounded group transition-colors", !isFinalized && "hover:bg-muted")} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        ...
        <div className="flex items-center gap-2 shrink-0 ml-4">
          ...
          {deductionAmount > 0 ? <span className="text-sm tabular-nums font-mono text-muted-foreground">
              −{formatAmount(deductionAmount, currency)}
            </span> : <span className="text-xs text-muted-foreground">No pay impact</span>}
        </div>
      </div>;
  }
  ...
  return <div className={cn("-mx-3 px-3 rounded transition-colors", expanded ? "bg-muted border border-border/30" : "hover:bg-muted")} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      ...
      toggleExpand();
        ...
        <div className="flex flex-col gap-0 min-w-0 flex-1">
          ...
          <AnimatePresence>
            {isHovered && !expanded && <motion.span initial={{
            height: 0,
            opacity: 0
          }} animate={{
            height: "auto",
        ...
        <span className="text-xs text-muted-foreground shrink-0 ml-3">
          {deductionAmount > 0 ? <span className="text-sm tabular-nums font-mono text-foreground">
              −{formatAmount(deductionAmount, currency)}
            </span> : 'No pay impact'}
        </span>
      </div>
...
export const CA3_SubmissionsView: React.FC<CA3_SubmissionsViewProps> = ({
  ...
  // Ready count includes all workers with zero pending/rejected items (naturally ready) + explicitly finalized
  ...
  const handleBulkApprove = () => {
    ...
    selectedSubmission.submissions.forEach((adj, idx) => {
      ...
      if (currentState.status === 'pending') {
        updateAdjustmentStatus(selectedSubmission.id, idx, {
          status: 'approved'
        });
      }
    });
  ...
  return <>
              ...
              <div className="flex sm:hidden items-center gap-1.5">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="min-w-[140px]">
                    ...
                    <DropdownMenuItem onClick={() => setActiveTab("pending")} className="text-xs">Pending ({pendingCount})</DropdownMenuItem>
      ...
      <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
        <SheetContent side="right" className="w-full sm:max-w-[420px] overflow-y-auto p-0" hideClose={isAddingAdjustment}>
          ...
          const moneyAdjustments = adjustmentEntries.filter(({
            adj
          }) => typeof adj.amount === 'number' && (adj.amount ?? 0) !== 0);
          ...
          const pendingAdjustmentCount = moneyAdjustments.filter(({
            adj,
            originalIdx
          }) => {
          ...
          const approvedAdjustmentTotal = moneyAdjustments.reduce((sum, {
            adj,
            originalIdx
          }) => {
            const effectiveStatus = getEffectiveStatus(originalIdx, adj.status as AdjustmentItemStatus);
          ...
          const getAdjustmentCounts = (types: string[]) => {
            const filtered = allAdjustments.map((adj, idx) => ({
              adj,
              idx
            })).filter(({
              adj
            }) => types.includes(adj.type));
            let pending = 0,
              approved = 0,
              rejected = 0;
            filtered.forEach(({
              ...
              if (status === 'pending') pending++;else if (status === 'approved') approved++;else if (status === 'rejected') rejected++;
            });
            return {
              pending,
              approved,
          ...
          return <>
                ...
                {isAddingAdjustment ? <CA3_AdminAddAdjustment workerType={selectedSubmission.workerType} workerName={selectedSubmission.workerName} currency={currency} dailyRate={selectedSubmission.basePay ? selectedSubmission.basePay / 22 : 100} hourlyRate={selectedSubmission.basePay ? selectedSubmission.basePay / 176 : 15} isOpen={isAddingAdjustment} onOpenChange={setIsAddingAdjustment} onAddAdjustment={adjustment => handleAdminAddAdjustment(selectedSubmission.id, adjustment)} /> : <>
                ...
                <SheetHeader className="px-5 pt-5 pb-4 border-b border-border/20">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {!showPendingOnly && !isWorkerFinalized(selectedSubmission.id) && !selectedSubmission.flags?.some(f => f.type === "end_date") && <button onClick={() => setIsAddingAdjustment(true)} className="flex items-center gap-1 px-2 py-0.5 text-[10px] text-muted-foreground border border-border/50 rounded-full hover:border-primary/40 hover:text-primary hover:bg-primary/5 transition-colors">
                  ...
                  {/* Net pay hero */}
                  {!isAddingAdjustment && <div className="mt-4 pt-4 border-t border-border/20">
                      <div className="flex items-end justify-between">
                        <div>
                          ...
                          <button onClick={() => setShowReceiptView(true)} className="text-[10px] text-muted-foreground/50 hover:text-primary transition-colors mt-0.5">
                            View receipt →
                          </button>
                        </div>
                        <CurrencyToggle
                ...
                <div className="px-5 py-4 space-y-0.5" onClick={() => setExpandedItemId(null)}>
                  ...
                  {(() => {
                    ...
                    {(() => {
                            const expenseBonusAdjs = allAdjustments.map((adj, originalIdx) => ({
                              adj,
                              originalIdx
                            })).filter(({
                              adj
                            }) => adj.type === 'expenses' || adj.type === 'bonus').filter(({
                              adj,
                              originalIdx
                            }) => {
                                  ...
                                  <GroupedExpenseRow
                                    ...
                                    items={groupItems.map(({ adj, originalIdx }) => {
                                      ...
                                      return {
                                        ...
                                        onApprove: () => { updateAdjustmentStatus(selectedSubmission.id, originalIdx, { status: 'approved' }); toast.success(`Approved expense`); },
                                        onReject: (reason: string) => { updateAdjustmentStatus(selectedSubmission.id, originalIdx, { status: 'rejected', rejectionReason: reason }); toast.info(`Rejected expense`); },
                    ...
                    {!showPendingOnly && workerAdminAdjustments.filter(a => a.type === 'expense').map(adj => <motion.div key={adj.id} initial={newlyAddedId === adj.id ? {
                            ...
                            duration: 0.25,
                            ease: "easeOut"
                          }} className={cn("rounded transition-all duration-500 group", newlyAddedId === adj.id ? "bg-primary/5 ring-1 ring-primary/20" : "-mx-3 px-3 hover:bg-muted/50")}>
                          <div className="flex items-center justify-between py-2">
                            ...
                            <div className="flex items-center">
                              ...
                              <button onClick={e => {
                                  e.stopPropagation();
                                  handleRemoveAdminAdjustment(selectedSubmission.id, adj.id);
                  ...
                  {(hasLeaves || workerAdminAdjustments.some(a => a.type === 'unpaid_leave')) && (!showPendingOnly || leaveCounts.pending > 0) && <CollapsibleSection title="Leave" defaultOpen={leaveCounts.pending > 0} forceOpen={showPendingOnly ? leaveCounts.pending > 0 : (leaveCounts.pending > 0 || newlyAddedSection === 'leave')} pendingCount={leaveCounts.pending} approvedCount={leaveCounts.approved + workerAdminAdjustments.filter(a => a.type === 'unpaid_leave').length}>
                    ...
                    {!showPendingOnly && workerAdminAdjustments.filter(a => a.type === 'unpaid_leave').map(adj => <motion.div key={adj.id} initial={newlyAddedId === adj.id ? {
                          opacity: 0,
                          y: -8,
                          scale: 0.98
                        } : false} animate={{
                  ...
                  {showCustomReason && <section className="space-y-3 pt-2 border-t border-border/40">
                      ...
                      <div className="space-y-2">
                        ...
                        <Textarea id="rejection-reason" placeholder="Add a short reason (visible to employee)" value={rejectReason} onChange={e => setRejectReason(e.target.value)} className="min-h-[80px] resize-none" />
                      </div>
                      ...
                      setShowCustomReason(false);
                    }} disabled={!rejectReason.trim()} className="flex-1 bg-destructive hover:bg-destructive/90 text-destructive-foreground">
                          Reject request
                        </Button>
                      </div>
                    </section>}
                ...
                return <div className="border-t border-border/30 bg-gradient-to-b from-transparent to-muted/20 px-5 py-4 space-y-2">
                      ...
                      <button
                        onClick={() => {
                          // Reset all adjustment and leave states back to pending
              ...
              return <CA3_MarkAsReadyDialog open={showMarkAsReadyDialog} onOpenChange={setShowMarkAsReadyDialog} onConfirm={handleMarkAsReady} workerName={selectedSubmission.workerName} approvedCount={approvedCount} rejectedCount={rejectedCount} />;
                ...
                <AnimatePresence>
                  {showReceiptView && <motion.div initial={{
                x: "100%"
              }} animate={{
                x: 0
              }} exit={{
                      ...
                      <div className="flex-1 overflow-y-auto px-5 py-4">
                        ...
                        <div className="mb-4">
                          ...
                          <div className="space-y-1.5">
                            {earnings.map((item, idx) => <div key={idx} className="flex justify-between text-sm">
                                <span className="text-muted-foreground">{item.label}</span>
                                <span className="tabular-nums font-mono">{formatCurrency(item.amount, currency)}</span>
                              </div>)}
                            {/* Approved adjustments */}
                            {allAdjustments.filter(adj => adj.type === 'expenses' || adj.type === 'bonus').map((adj, idx) => {
                        ...
                        const config = submissionTypeConfig[adj.type as SubmissionType];
                        return <div key={`adj-${idx}`} className="flex justify-between text-sm">
                        ...
                        {/* Deductions Section */}
                        {deductions.length > 0 && <div className="mb-4">
                            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Deductions</h3>
                            <div className="space-y-1.5">
                              {deductions.map((item, idx) => <div key={idx} className="flex justify-between text-sm">
                                  <span className="text-muted-foreground">{item.label}</span>
                        ...
                        <div className="bg-primary/5 rounded-lg p-4 border border-primary/20">
                          ...
                          {(approvedAdjustmentTotal !== 0 || approvedLeaveDeduction !== 0) && <p className="text-xs text-muted-foreground mt-1 text-right">
