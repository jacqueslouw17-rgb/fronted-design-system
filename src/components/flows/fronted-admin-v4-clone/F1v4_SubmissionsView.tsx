                ...
                <TooltipTrigger asChild>
                  <span>
                    <Button size="sm" onClick={() => {
                      // Compute ready worker IDs and excluded worker IDs
                      ...
                      submissions.forEach(s => {
                        ...
                        // Check if naturally ready (no pending/rejected)
                        const pendingAdjs = s.submissions.filter((adj, idx) => {
                          const key = `${s.id}-${idx}`;
                          const localState = adjustmentStates[key];
                          const effectiveStatus = localState?.status || adj.status || 'pending';
                ...
                <>
                  ...
                  <div className="px-5 py-2.5 border-b border-border/20">
                    ...
                    <div className={cn("px-5 py-4 space-y-0.5", statusDecisions[selectedSubmission.id] === "exclude" && "opacity-40 pointer-events-none line-through")} onClick={() => setExpandedItemId(null)}>
                      ...
                      <CollapsibleSection title="Leave" defaultOpen={leaveCounts.pending > 0} forceOpen={showPendingOnly ? leaveCounts.pending > 0 : (leaveCounts.pending > 0 || newlyAddedSection === 'leave')} pendingCount={leaveCounts.pending} approvedCount={leaveCounts.approved + workerAdminAdjustments.filter((a) => a.type === 'unpaid_leave').length}>
                    ...
                    // Show finalized state with undo option
                    return (
