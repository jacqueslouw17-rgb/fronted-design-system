/**
 * CA4_PeriodDropdown — Premium period selector with off-cycle batch support
...
const StatusDot = ({ status, size = "sm", isCustom = false }: { status: DotStatus; size?: "sm" | "md"; isCustom?: boolean }) => {
...
const statusLabel = (s: DotStatus) =>
  s === "paid" ? "Paid" : s === "processing" ? "Processing" : "In review";

export const CA4_PeriodDropdown: React.FC<CA4_PeriodDropdownProps> = ({
  periods,
  selectedPeriodId,
  onPeriodChange,
}) => {
  ...
  const triggerLabel = selectedPeriod
    ? selectedPeriod.isCustomBatch
      ? `Custom · ${selectedPeriod.periodLabel || selectedPeriod.label}`
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      "hover:bg-muted/60",
                      isSelected && period.status === "current" && "bg-amber-500/[0.06] ring-1 ring-amber-500/15",
                  ...
                  >
                    <div className={cn(
                      ...
                      isSelected
                        ? period.status === "current" ? "bg-amber-500" : "bg-primary"
                        : "bg-transparent"
                    )} />
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className={cn("text-[13px] font-medium", isSelected ? "text-foreground" : "text-foreground/70")}>
                            {period.periodLabel || period.label}
                          </span>
                        </div>
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <span className={cn(
                          "text-[11px] font-medium",
                          period.status === "current" && "text-violet-600 dark:text-violet-400",
...
export default CA4_PeriodDropdown;
