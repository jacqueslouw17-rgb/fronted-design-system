...
const CompanyAdministratorsDetail = ({
  onCancel
}: CompanyAdministratorsDetailProps) => {
      ...
      <div className="bg-card/40 border border-border/40 rounded-lg p-6">
        ...
        <div className="space-y-3">
          <AnimatePresence mode="popLayout">
            ...
            return <motion.div key={admin.id} initial={{
              opacity: 0,
              y: -10
            }} animate={{
              opacity: 1,
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="flex gap-4 text-xs text-muted-foreground">
                        ...
                        {admin.lastLoginDate && <span>Last login: {new Date(admin.lastLoginDate).toLocaleDateString()}</span>}
                      </div>
                    </div>
                    <Button variant="outline" size="sm" onClick={() => handleResendPassword(admin.id, admin.companyName)} disabled={resendingId === admin.id} className="ml-4 flex-shrink-0">
