...
export function RolesPermissionsSection({
  ...
  const RoleCard = ({
    ...
    return <Card className="p-4 bg-card border-border/40 hover:bg-muted/30 transition-colors">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2.5 mb-1">
              ...
              {role.is_system_role && <Badge variant="secondary" className="gap-1 text-xs">
          ...
          {canManageRoles && <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {!role.is_system_role && <DropdownMenuItem onClick={() => handleEditRole(role)}>
              ...
              setDuplicateName(`${role.name} (Copy)`);
                ...
                {!role.is_system_role && <>
                    ...
                    <DropdownMenuItem className="text-destructive focus:text-destructive" onClick={() => setRoleToDelete(role)} disabled={!canDelete}>
                      <Trash2 className="h-4 w-4 mr-2" />
                      Delete
                      {memberCount > 0 && <span className="ml-auto text-xs opacity-60">
  ...
  return <div className="space-y-6">
      ...
      <Card className="p-6 bg-card/50 backdrop-blur-sm border-border/40">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {canManageRoles && <Button onClick={handleCreateRole} className="gap-1.5">
        ...
        {/* System Roles */}
        {systemRoles.length > 0 && <div className="mb-6">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-3">
              System Roles
            </p>
            <div className="space-y-2">
              <AnimatePresence mode="popLayout">
                {systemRoles.map(role => <motion.div key={role.id} initial={{
              opacity: 0,
              y: -10
            }} animate={{
              opacity: 1,
        ...
        <div>
          ...
          {customRoles.length > 0 ? <div className="space-y-2">
              <AnimatePresence mode="popLayout">
                {customRoles.map(role => <motion.div key={role.id} initial={{
              opacity: 0,
              y: -10
            }} animate={{
              opacity: 1,
      ...
      {/* Role Editor Modal */}
      <RoleEditorModal open={showRoleEditor} onOpenChange={setShowRoleEditor} modules={modules} role={editingRole} onSave={handleSaveRole} getPermissionSummary={getPermissionSummary} />
      ...
      <Dialog open={!!roleToDuplicate} onOpenChange={() => setRoleToDuplicate(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-4">
            ...
            <Input id="duplicateName" value={duplicateName} onChange={e => setDuplicateName(e.target.value)} className="mt-2" placeholder="Enter role name" />
          </div>
          <DialogFooter>
