...
import { FileText, Handshake, ScrollText, ChevronLeft, ChevronRight, Package } from "lucide-react";
  ...
  return <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={handlePrev} disabled={currentIndex === 0} className="h-8 w-8 p-0">
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <span className="text-sm font-medium text-foreground min-w-[3rem] text-center">
            {currentIndex + 1} / {documents.length}
          </span>
          <Button variant="ghost" size="sm" onClick={handleNext} disabled={currentIndex === documents.length - 1} className="h-8 w-8 p-0">
      ...
      <div className="relative h-[220px] overflow-hidden">
        <AnimatePresence mode="wait">
          ...
          duration: 0.3
            ...
            <Card className="h-full px-6 py-4 border border-border/40 bg-card/50 backdrop-blur-sm">
                    ...
                    <div className="space-y-0.5">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="text-foreground font-medium">
                          {documents[currentIndex].id === "employment-agreement" ? "8" : documents[currentIndex].id === "nda" ? "3" : "2"}
                        </span>
                      </div>
                      <div className="flex justify-between text-xs">
      ...
      <div className="flex justify-center gap-2">
        {documents.map((doc, idx) => <button key={doc.id} onClick={() => setCurrentIndex(idx)} className={`h-2 rounded-full transition-all ${idx === currentIndex ? "w-8 bg-primary" : doc.selected ? "w-2 bg-primary/30" : "w-2 bg-border"}`} />)}
      </div>

      {/* Generate bundle button */}
      {!hideButton && <Button onClick={handleGenerate} className="w-full" size="lg">
