...
import { F1v5_ContractDraftWorkspace } from "@/components/flows/entity-v1/F1v7_ContractDraftWorkspace";
...
import { F1v4_PipelineView } from "@/components/flows/entity-v1/F1v7_PipelineView";
...
import F1v4_EmbeddedAdminOnboarding from "@/components/flows/entity-v1/F1v7_EmbeddedAdminOnboarding";
import { F1v4_AddCandidateDrawer } from "@/components/flows/entity-v1/F1v7_AddCandidateDrawer";
import { F1v4_PayrollTab } from "@/components/flows/entity-v1/F1v7_PayrollTab";
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { F1v7_PrioritiesTab } from "@/components/flows/entity-v1/priorities/F1v7_PrioritiesTab";
import { F1v7_KurtPanel } from "@/components/flows/entity-v1/priorities/F1v7_KurtPanel";

import type { ActionDetail } from "@/components/flows/entity-v1/priorities/F1v7_PriorityData";
...
const FLOW_BASE_PATH = "/flows/entity-v1";
...
const AdminContractingMultiCompany = () => {
  ...
  const [selectedCompany, setSelectedCompany] = useState<string>(() => {
    ...
    const saved = localStorage.getItem('entity-v1-selected-company');
  ...
  const [companies, setCompanies] = useState<CompanyData[]>(() => {
    const saved = localStorage.getItem('entity-v1-companies');
    if (saved) {
      try {
        ...
        if (missing.length > 0) {
          ...
          localStorage.setItem('entity-v1-companies', JSON.stringify(merged));
          return merged;
        }
        return parsed;
      } catch {
  ...
  const [companyContractors, setCompanyContractors] = useState<Record<string, any[]>>(() => {
    const saved = localStorage.getItem('entity-v1-company-contractors');
    if (saved) {
      try {
        ...
        localStorage.removeItem('entity-v1-company-contractors');
      }
    }
    return {
      "company-default": [...DEFAULT_DRAFTING_CANDIDATES],
  ...
  useEffect(() => {
    localStorage.setItem('entity-v1-companies', JSON.stringify(companies));
  }, [companies]);
  
  // Persist selected company to localStorage
  useEffect(() => {
    localStorage.setItem('entity-v1-selected-company', selectedCompany);
  }, [selectedCompany]);
  
  // Persist company contractors to localStorage
  useEffect(() => {
    localStorage.setItem('entity-v1-company-contractors', JSON.stringify(companyContractors));
  }, [companyContractors]);
  ...
  const handleBackToFlows = () => {
    localStorage.removeItem('entity-v1-companies');
    localStorage.removeItem('entity-v1-selected-company');
    localStorage.removeItem('entity-v1-company-contractors');
  };
    ...
    <RoleLensProvider initialRole="admin">
      ...
      {!isAddingNewCompany && !isEditingCompany && (
        ...
        <Topbar 
          ...
          profileSettingsUrl="/entity-v1/profile-settings"
