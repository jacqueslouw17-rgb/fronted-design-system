...
const Flows = () => {
  ...
  const flows = [
    ...
    },
    {
      ...
      description: "Duplicate of Flow 5 for contractor-specific payout views. Will later be updated to support contractor-specific payout views. Currently a clean clone — no modifications yet.",
      steps: 2,
      patterns: ["data-cards", "compliance-checklist", "genie-onboarding"],
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="space-y-8">
          {frontedFlows.length > 0 && (
            <CollapsibleFlowGroup label="Fronted">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {frontedFlows.map((flow) => renderFlowCard(flow))}
              </div>
            </CollapsibleFlowGroup>
          )}
          {endClientFlows.length > 0 && (
            <CollapsibleFlowGroup label="End-client">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {endClientFlows.map((flow) => renderFlowCard(flow))}
              </div>
            </CollapsibleFlowGroup>
          )}
