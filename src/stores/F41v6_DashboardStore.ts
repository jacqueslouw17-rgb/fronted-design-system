/**
 * Flow 4.1 — Employee Dashboard v6 Store
 * 
 * Namespaced store for Employee Dashboard v6.
 * Includes 5-status payroll states and adjustment tracking.
 * INDEPENDENT from v5 and all other flows - changes here do not affect other flows.
 */
...
export interface F41v6_LineItem {
...
export interface F41v6_EmployerCost {
  label: string;
  amount: number;
}

export interface F41v6_Adjustment {
  id: string;
  type: F41v6_AdjustmentType;
  label: string;
  amount: number | null;
...
export interface F41v6_LeaveRequest {
  id: string;
  leaveType: F41v6_LeaveType;
  startDate: string;
  endDate: string;
  totalDays: number;
  reason?: string;
  status: F41v6_LeaveStatus;
  submittedAt: string;
  rejectionReason?: string;
}

interface F41v6_DashboardState {
  dashboard_context: 'employee_v6';
  isLoading: boolean;
  ...
  lineItems: F41v6_LineItem[];
  employerCosts: F41v6_EmployerCost[];
  windowState: F41v6_WindowState;
  
  // New 5-status state
  payrollStatus: F41v6_PayrollStatus;
  ...
  adjustments: F41v6_Adjustment[];
  leaveRequests: F41v6_LeaveRequest[];
...
interface F41v6_DashboardActions {
  setLoading: (loading: boolean) => void;
  submitForReview: () => void;
  submitNoChanges: () => void;
  fixAndResubmit: () => void;
  setPayrollStatus: (status: F41v6_PayrollStatus) => void;
  confirmPay: () => void;
  withdrawSubmission: () => void;
  addAdjustment: (adjustment: Omit<F41v6_Adjustment, 'id' | 'submittedAt' | 'status'>) => void;
  addLeaveRequest: (leave: Omit<F41v6_LeaveRequest, 'id' | 'submittedAt' | 'status'>) => void;
...
const initialState: F41v6_DashboardState = {
  dashboard_context: 'employee_v6',
  isLoading: false,
  ...
  adjustments: [
    // Mock pending adjustment
    {
      ...
      type: 'Expense' as F41v6_AdjustmentType,
      label: 'Team lunch',
      amount: 2500,
    ...
    // Mock rejected adjustment with reason
    {
      ...
      type: 'Overtime' as F41v6_AdjustmentType,
      label: 'Weekend work',
      amount: 3500,
      status: 'Admin rejected' as F41v6_AdjustmentStatus,
      hours: 8,
      date: '2026-01-11',
      startTime: '09:00',
      endTime: '17:00',
  ...
  leaveRequests: [
    ...
    {
      id: 'leave-demo-1',
      leaveType: 'Vacation' as F41v6_LeaveType,
      startDate: '2026-01-15',
      endDate: '2026-01-15',
      totalDays: 1,
      status: 'Admin approved' as F41v6_LeaveStatus,
    ...
    // Sample pending leave
    {
      ...
      endDate: '2026-02-17',
      totalDays: 2,
      status: 'Pending' as F41v6_LeaveStatus,
      submittedAt: '2026-01-10T10:00:00.000Z',
    },
    // Sample rejected leave with reason
    {
      id: 'leave-demo-3',
      leaveType: 'Sick' as F41v6_LeaveType,
      startDate: '2026-01-25',
      endDate: '2026-01-26',
      totalDays: 2,
      status: 'Admin rejected' as F41v6_LeaveStatus,
...
export const useF41v6_DashboardStore = create<F41v6_DashboardState & F41v6_DashboardActions>((set) => ({
  ...initialState,
  ...
  addAdjustment: (adjustment) =>
    set((state) => {
      ...
      const nextPayrollStatus: F41v6_PayrollStatus = isApprovedOrLater ? 'submitted' : state.payrollStatus;
