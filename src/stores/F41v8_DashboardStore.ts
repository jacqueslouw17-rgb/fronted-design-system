...
export interface F41v8_LineItem {
...
export interface F41v8_EmployerCost {
  label: string;
  amount: number;
}

export interface F41v8_Adjustment {
  id: string;
  type: F41v8_AdjustmentType;
  label: string;
  amount: number | null;
  status: F41v8_AdjustmentStatus;
  description?: string;
  category?: string;
  days?: number;
  hours?: number;
...
export interface F41v8_LeaveRequest {
  id: string;
  leaveType: F41v8_LeaveType;
  startDate: string;
  endDate: string;
  totalDays: number;
  reason?: string;
  status: F41v8_LeaveStatus;
  submittedAt: string;
  rejectionReason?: string;
}

interface F41v8_DashboardState {
  ...
  lineItems: F41v8_LineItem[];
  employerCosts: F41v8_EmployerCost[];
  windowState: F41v8_WindowState;
  payrollStatus: F41v8_PayrollStatus;
  returnedReason?: string;
  resubmitDeadline?: string;
  submittedAt?: string;
  approvedAt?: string;
  confirmed: boolean;
  adjustments: F41v8_Adjustment[];
  leaveRequests: F41v8_LeaveRequest[];
...
interface F41v8_DashboardActions {
  setLoading: (loading: boolean) => void;
  submitForReview: () => void;
  submitNoChanges: () => void;
  fixAndResubmit: () => void;
  setPayrollStatus: (status: F41v8_PayrollStatus) => void;
  confirmPay: () => void;
  withdrawSubmission: () => void;
  addAdjustment: (adjustment: Omit<F41v8_Adjustment, 'id' | 'submittedAt' | 'status'>) => void;
  addLeaveRequest: (leave: Omit<F41v8_LeaveRequest, 'id' | 'submittedAt' | 'status'>) => void;
...
const initialState: F41v8_DashboardState = {
  dashboard_context: 'employee_v7',
  isLoading: false,
  employeeCountry: 'NO',
  ...
  adjustments: [
    ...
    },
    {
      ...
      type: 'Expense' as F41v8_AdjustmentType,
    ...
    },
    {
      ...
      type: 'Overtime' as F41v8_AdjustmentType,
      label: '8h · Jan 11 · 09:00–17:00',
      amount: 3500,
      status: 'Admin rejected' as F41v8_AdjustmentStatus,
      hours: 8,
      date: '2026-01-11',
      startTime: '09:00',
      endTime: '17:00',
  ...
  leaveRequests: [
    {
      id: 'leave-demo-1',
      leaveType: 'Vacation' as F41v8_LeaveType,
      startDate: '2026-01-15',
      endDate: '2026-01-15',
      totalDays: 1,
      status: 'Admin approved' as F41v8_LeaveStatus,
    ...
    },
    {
      ...
      endDate: '2026-02-17',
      totalDays: 2,
      status: 'Pending' as F41v8_LeaveStatus,
      submittedAt: '2026-01-10T10:00:00.000Z',
    },
    {
      id: 'leave-demo-3',
      leaveType: 'Sick' as F41v8_LeaveType,
      startDate: '2026-01-25',
      endDate: '2026-01-26',
      totalDays: 2,
      status: 'Admin rejected' as F41v8_LeaveStatus,
...
export const useF41v8_DashboardStore = create<F41v8_DashboardState & F41v8_DashboardActions>((set) => ({
  ...initialState,
  ...
  addAdjustment: (adjustment) =>
    set((state) => {
      ...
      const nextPayrollStatus: F41v8_PayrollStatus = isApprovedOrLater ? 'submitted' : state.payrollStatus;
      return {
