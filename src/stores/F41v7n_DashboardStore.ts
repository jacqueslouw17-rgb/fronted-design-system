/**
 * Flow 4.1 — Employee Dashboard v8 Store
 * 
 * Namespaced store for Employee Dashboard v8.
...
export type F41v7n_AdjustmentLeaveType = 'Paid leave' | 'Unpaid leave' | 'Sick leave' | 'Maternity / parental leave' | 'Other leave';
...
export interface F41v7n_LineItem {
...
export interface F41v7n_EmployerCost {
  label: string;
  amount: number;
}

export interface F41v7n_Adjustment {
  id: string;
  type: F41v7n_AdjustmentType;
  label: string;
  amount: number | null;
  status: F41v7n_AdjustmentStatus;
  description?: string;
  category?: string;
  days?: number;
  hours?: number;
...
export interface F41v7n_LeaveRequest {
  id: string;
  leaveType: F41v7n_LeaveType;
  startDate: string;
  endDate: string;
  totalDays: number;
  reason?: string;
  status: F41v7n_LeaveStatus;
  submittedAt: string;
  rejectionReason?: string;
}

interface F41v7n_DashboardState {
  ...
  lineItems: F41v7n_LineItem[];
  employerCosts: F41v7n_EmployerCost[];
  windowState: F41v7n_WindowState;
  payrollStatus: F41v7n_PayrollStatus;
  returnedReason?: string;
  resubmitDeadline?: string;
  submittedAt?: string;
  approvedAt?: string;
  confirmed: boolean;
  adjustments: F41v7n_Adjustment[];
  leaveRequests: F41v7n_LeaveRequest[];
...
interface F41v7n_DashboardActions {
  setLoading: (loading: boolean) => void;
  submitForReview: () => void;
  submitNoChanges: () => void;
  fixAndResubmit: () => void;
  setPayrollStatus: (status: F41v7n_PayrollStatus) => void;
  confirmPay: () => void;
  withdrawSubmission: () => void;
  addAdjustment: (adjustment: Omit<F41v7n_Adjustment, 'id' | 'submittedAt' | 'status'>) => void;
  addLeaveRequest: (leave: Omit<F41v7n_LeaveRequest, 'id' | 'submittedAt' | 'status'>) => void;
...
const initialState: F41v7n_DashboardState = {
  dashboard_context: 'employee_v7',
  isLoading: false,
  employeeCountry: 'NO',
  ...
  adjustments: [
    ...
    },
    {
      ...
      type: 'Expense' as F41v7n_AdjustmentType,
    ...
    },
    {
      ...
      type: 'Overtime' as F41v7n_AdjustmentType,
      label: '8h · Jan 11 · 09:00–17:00',
      amount: 3500,
      status: 'Admin rejected' as F41v7n_AdjustmentStatus,
      hours: 8,
      date: '2026-01-11',
      startTime: '09:00',
      endTime: '17:00',
  ...
  leaveRequests: [
    {
      id: 'leave-demo-1',
      leaveType: 'Vacation' as F41v7n_LeaveType,
      startDate: '2026-01-15',
      endDate: '2026-01-15',
      totalDays: 1,
      status: 'Admin approved' as F41v7n_LeaveStatus,
    ...
    },
    {
      ...
      endDate: '2026-02-17',
      totalDays: 2,
      status: 'Pending' as F41v7n_LeaveStatus,
      submittedAt: '2026-01-10T10:00:00.000Z',
    },
    {
      id: 'leave-demo-3',
      leaveType: 'Sick' as F41v7n_LeaveType,
      startDate: '2026-01-25',
      endDate: '2026-01-26',
      totalDays: 2,
      status: 'Admin rejected' as F41v7n_LeaveStatus,
...
export const useF41v7n_DashboardStore = create<F41v7n_DashboardState & F41v7n_DashboardActions>((set) => ({
  ...initialState,
  ...
  addAdjustment: (adjustment) =>
    set((state) => {
      ...
      const nextPayrollStatus: F41v7n_PayrollStatus = isApprovedOrLater ? 'submitted' : state.payrollStatus;
      return {
